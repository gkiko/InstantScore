...
public class DataParser {
    ...
    public static HashMap<String, ArrayList<Game>> getHashMapFromScoresFile(String data) throws Exception {
        ...
        for (int i = 0; i < numTourn; i++) {
            ...
            for (int j = 0; j < numMatches; j++) {
                ...
                if(needsConversion(time)) {
                    try {
                        Calendar cal = Utils.convertDateTime(Utils.buildDateTime(date, time));
                        time = Utils.getTimeFrom(cal);
                        date = Utils.getDateFrom(cal);
                    } catch (Exception e) {
                        Calendar calOnlyTime = Utils.convertDateTime(Utils.buildTime(time));
                        time = Utils.getTimeFrom(calOnlyTime);
                        date = "";
                    }
                }else{
                    date = "";
    ...
    private static  boolean needsConversion(String time){
